...
<div>
      ...
      <!-- Error Alert -->
      <Alert v-if="Object.keys(errors).length" class=" text-sm">
        <div v-for="(field, i) of Object.keys(errors)" :key="i" class="">
          <div v-for="(error, ind) of errors[field] || []" :key="ind">
            * {{error}}
          </div>
        </div>
      <span @click="errors=''" class="cursor-pointer w-8- h-8 flex items-center justify-center rounded-full transition-colors  hover:bg-[rgba(0,0,0,0.2)]"><svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <div>
          <button :class="{'cursor-not-allowed':loading,'hover:bg-indigo-500':loading}" :disabled="loading" type="submit" class="group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
